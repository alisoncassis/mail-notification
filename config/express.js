...
app.get('/:environment/web/lp/:name', (req, res) => res.sendFile(path.join(__dirname, `../static/web/lp/${req.params.name != 'null' ? req.params.name : 'default'}.html`)));
...
app.post('/:environment/error-report', (req, res) => {
    mailService.send({ body: req.body, type: 'errorReport', internal: true, env: req.params.environment });
    res.status(200).send();
});
